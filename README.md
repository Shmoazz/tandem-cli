...
Base URL `https://app.tandemai.io/api/v1`. Requirements live at `/programs/{p}/req-tables/{t}/requirements[/{id}]`. Create is POST, list is GET, update is PUT with a partial body (PATCH is not supported), and delete is DELETE with `{"deletion_reason": "..."}`. Status values are lowercase: `draft`, `review`, `approved`, `rejected`. The hierarchy is `parent_id`; the number is a label and implies nothing. PUT can change `number` and `parent_id`, so renumbering and re-parenting never need a delete.

Known Tandem bug, as of August 2026: requirement rows come back with a null `program_id`, so `/requirements/{id}/artifacts/from-node-artifact` answers "Requirement does not belong to this program" and a verification artifact can only be linked from the web app.
